...
api.on("ratelimit", ({ error: _error, method }: { error: RpcError; method: MethodInfo<any, any>; i: object; options: RpcOptions }) => {
...
onMounted(async () => {
	try {
		...
		session.value = undefined;
		await router.push({ name: "serverselect" });
		return;
	}
	...
	<div v-else class="h-full w-full flex">
		...
		<div class="bg-surface-1000 flex-1 flexcol overflow-hidden">
			<base-app-bar class="bg-surface-700 h-12 text-sm font-semibold">
				<base-button variant="text" icon dense class="!lg:hidden mr-2" :aria-label="$t('left-drawer')" @click="leftDrawer = true">
					<mdi-menu />
				</base-button>
				<mdi-pound class="text-lg mr-2 text-gray-400" />
				{{ channelData.data?.name }}
				<div class="flex-1" />
				<base-button variant="text" icon dense class="!lg:hidden" :aria-label="$t('right-drawer')" @click="rightDrawer = true">
			...
			<div v-else class="flex-1 flexcol gap-4 justify-center items-center">
				...
				<div class="bg-surface-800 rounded-full p-2 px-4">
					<p v-if="selectedGuild">{{ $t("select-a-channel-to-start-chatting") }}</p>
					<p v-else>{{ $t("select-a-guild-to-get-started") }}</p>
				</div>
			</div>
		</div>
			...
			<div class="flex h-full">
				<div class="w-48 flexcol bg-surface-900 border-l-2 border-surface-800">
					<div class="h-12 flex items-center p-3">
						<h3 class="text-xs uppercase text-gray-400">{{ $t("members") }}</h3>
					</div>
					...
					<base-menu
						...
						:offset="0"
						:options="[
							{ text: 'Settings', level: 'plain', onClick: openUserSettings },
							{ text: 'Log Out', level: 'plain', onClick: logOut },
						]"
					>
