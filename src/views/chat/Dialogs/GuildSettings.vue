...
const guildData = computed(() => chatState.getGuild(props.host, props.guildId).data);
...
<template>
	<form class="flexcol gap-8 w-90vw md:w-140" @submit.prevent>
		...
		<div class="flexcol gap-8 md:flex-row w-full justify-center">
			...
			<div>
				<base-input
					:model-value="displayValues.name"
					outlined
					:placeholder="$t('guild-name')"
					@input="(event) => changedValues.name = (event.target as HTMLInputElement).value"
				/>
			</div>
		</div>
		<div class="flex justify-end gap-2">
			...
			<base-button variant="text" color="primary" type="submit" :disabled="isDirty" @click="save">
				{{ $t("save") }}
			</base-button>
		</div>
	</form>
