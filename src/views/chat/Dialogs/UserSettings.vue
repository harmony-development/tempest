<template>
	<form class="p-2 flexcol gap-4" @submit.prevent="onSubmit">
		<div class="flex justify-center">
			<image-input v-model="changedValues.avatar" :preview-src="avatarURL" :fallback="profile?.username.charAt(0) || '?'" />
		</div>
		<base-input
			name="username"
			:type="'username'"
		...
		/>
		<base-checkbox name="Bot" :model-value="displayValues.bot" @update:model-value="changedValues.bot = $event" />
...
const dirty = computed(() => Object.entries(changedValues.value).every(([key, value]) => defaultValues.value[key] === value));
